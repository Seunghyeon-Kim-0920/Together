import { BedDouble, Bus, FileDown, Landmark, Plus, ReceiptText, Share2, ShoppingBag, Trash2, Utensils, UserPlus, WalletCards } from "lucide-react";
import { Capacitor } from "@capacitor/core";
import { useMemo, useState } from "react";
import { availableCurrencies, currencyName, formatMoney, parseMinorUnits } from "../lib/currency";
import { t, travelCategoryLabel } from "../lib/i18n";
import { shareTravelLedger, safeFilename } from "../lib/share";
...
import { createTravelExpense, defaultDate, settleTravelExpenses } from "../lib/wallet";
...
export function TravelLedgerView({ ledger, locale, onChange, onNotify }: { ledger: TravelLedger; locale: Locale; onChange: (ledger: TravelLedger) => void; onNotify: Notify }) {
  const [formOpen, setFormOpen] = useState(false); const [participantName, setParticipantName] = useState(""); const [filter, setFilter] = useState<TravelCategory | "all">("all"); const [currencyToAdd, setCurrencyToAdd] = useState("USD"); const [currencyEditorOpen, setCurrencyEditorOpen] = useState(false);
  ...
  const share = async () => { try { await shareTravelLedger(ledger); onNotify(t(locale, "sharedFileReady"), "success"); } catch { onNotify(t(locale, "shareFailed"), "error"); } };
  const exportPdf = async () => {
    try {
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        await Share.share({ title: ledger.title, url: written.uri, dialogTitle: t(locale, "exportPdf") });
      } else {
        pdf.save(filename);
      }
  ...
  return (
    <section className="ledger-screen">
      ...
      <section className="mobile-panel expense-history"><div className="section-heading"><h3>{t(locale, "recentExpenses")}</h3><button className="primary-compact" type="button" onClick={() => setFormOpen(true)} disabled={ledger.participants.length === 0}><Plus />{t(locale, "addExpense")}</button></div>
        ...
        {visibleExpenses.length ? <div className="expense-list">{[...visibleExpenses].sort((a, b) => b.occurredOn.localeCompare(a.occurredOn)).map((expense) => <article key={expense.id}><span className="category-icon"><CategoryIcon category={expense.category} /></span><div><strong>{expense.description}</strong><small>{travelCategoryLabel(locale, expense.category)} · {participantNames.get(expense.paidBy)} · {expense.occurredOn}</small></div><b>{formatMoney(expense.minorUnits, expense.currency, locale)}</b><button type="button" aria-label={t(locale, "delete")} onClick={() => removeExpense(expense.id)}><Trash2 /></button></article>)}</div> : <p className="ledger-empty">{t(locale, "noExpenses")}</p>}
      </section>
      <button className="floating-add" type="button" onClick={() => setFormOpen(true)} disabled={ledger.participants.length === 0}><Plus />{t(locale, "addExpense")}</button>
      {formOpen ? <TravelExpenseSheet ledger={ledger} locale={locale} onClose={() => setFormOpen(false)} onAdd={(expense) => { update({ expenses: Object.freeze([...ledger.expenses, expense]) }); setFormOpen(false); onNotify(t(locale, "expenseAdded"), "success"); }} onNotify={onNotify} /> : null}
