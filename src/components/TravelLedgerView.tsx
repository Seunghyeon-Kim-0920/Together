...
import { createTravelExpense, defaultDate, newestExpensesFirst, preserveTravelShares, replaceExpenseById, settleTravelExpenses } from "../lib/wallet";
...
function TravelExpenseSheet({ ledger, locale, expense, onClose, onSave, onNotify }: { ledger: TravelLedger; locale: Locale; expense: TravelExpense | null; onClose: () => void; onSave: (expense: TravelExpense) => void; onNotify: Notify }) {
  ...
  const submit = () => { const minorUnits = parseMinorUnits(amount, currency); if (!description.trim() || !paidBy || selected.length === 0 || !occurredOn) return onNotify(t(locale, "requiredFields"), "error"); if (!minorUnits) return onNotify(t(locale, "amountInvalid"), "error"); const saved = createTravelExpense({ description: description.trim(), category, currency, minorUnits, paidBy, participantIds: selected, occurredOn }); const preservedShares = expense ? preserveTravelShares(expense, minorUnits, currency, selected) : null; onSave(expense ? Object.freeze({ ...saved, id: expense.id, ...(preservedShares ? { shares: preservedShares } : {}) }) : saved); };
